...
@NewestPostID = null
@BuildPostQueue = ()->
  if @FetchingPosts or @PostQueue.length is 50 or @RequestCount >= @RequestLimit
    return
  @FetchingPosts = true
  #Retreive the last done post
  request = Reddit.list().hot().sort('top').from('day').limit(50) #request so far, only process the ones of this week
